...
describe('Contract: cast-vote WebSocket Event', () => {
  ...
  describe('Successful Vote Flow', () => {
    it('should accept valid cast-vote request and emit vote-accepted', async () => {
      const voteAcceptedPromise = new Promise<any>((resolve) => {
    ...
    it('should broadcast vote-count-updated to all participants', async () => {
      ...
      const updates = await Promise.all([
        new Promise<any>((resolve) => socket1.once('vote-count-updated', resolve)),
        new Promise<any>((resolve) => socket2.once('vote-count-updated', resolve)),
        new Promise<any>((resolve) => socket3.once('vote-count-updated', resolve))
    ...
    it('should accept all valid card values (1,2,3,5,8,13,21,?)', async () => {
      const cardValues: Array<1 | 2 | 3 | 5 | 8 | 13 | 21 | '?'> = [1, 2, 3, 5, 8, 13, 21, '?'];
