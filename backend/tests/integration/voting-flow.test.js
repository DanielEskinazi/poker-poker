    ...
    describe('Complete Voting Workflow', () => {
        it('should handle complete voting flow: vote → count → reveal → statistics', async () => {
            ...
            const revealPromises = [
                ...
                new Promise((resolve) => participant3Socket.once('votes-revealed', resolve))
            ];
            ...
            const voteValues = modRevealed.votes.map((v) => v.cardValue).sort((a, b) => a - b);
        ...
        it('should allow vote changes before reveal', async () => {
            ...
            const p1Vote = revealed.votes.find((v) => v.participantId === participant1Id);
...
//# sourceMappingURL=voting-flow.test.js.map