...
describe('Integration: Multi-User Join Flow', () => {
  ...
  it('should allow multiple users to join session sequentially with real-time updates', async () => {
    ...
    const sessionCreated = new Promise<{ sessionId: string }>((resolve) => {
    ...
    for (let i = 0; i < participantNames.length; i++) {
      ...
      const joinAccepted = new Promise<any>((resolve) => {
  ...
  it('should synchronize participant list across all connected clients', async () => {
    ...
    const creatorJoins: string[] = [];
    const participant1Joins: string[] = [];
