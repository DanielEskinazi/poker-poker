...
describe('Integration: Reset Flow', () => {
    ...
    it('should handle complete voting cycle: vote → reveal → reset → new vote', async () => {
        ...
        // Reveal round 1
        const reveal1Promise = new Promise((resolve) => moderatorSocket.once('votes-revealed', resolve));
        ...
        const resetPromises = [
            new Promise((resolve) => moderatorSocket.once('votes-reset', resolve)),
            new Promise((resolve) => participant1Socket.once('votes-reset', resolve)),
            new Promise((resolve) => participant2Socket.once('votes-reset', resolve))
        ];
        ...
        await new Promise(resolve => setTimeout(resolve, 100));
        // Reveal round 2
        const reveal2Promise = new Promise((resolve) => moderatorSocket.once('votes-revealed', resolve));
        moderatorSocket.emit('reveal-votes', { sessionId, moderatorId });
        const revealed2 = await reveal2Promise;
        // Verify only new votes are shown (2 votes: 13, 21)
        expect(revealed2.votes).toHaveLength(2);
        const voteValues = revealed2.votes.map((v) => v.cardValue).sort((a, b) => a - b);
...
//# sourceMappingURL=reset-flow.test.js.map